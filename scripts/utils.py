...
def making_rgb_numerically(row, bo, ds, scaling=SCALING, verbose = False, image_type = TYPE_OF_IMAGE, RANDOM_OR=RANDOM_OR, MARGIN=MARGIN):
    ...
    if image_type == 'A':
        ...
        r = distance(cords, n_atoms)
        g = coulomb_matrix(cords, n_atoms, atom_types, diagonal = False)
        b = g2_matrix(cords, 1, 1, 5)

    elif image_type == 'plain_red':
        r = distance(cords, n_atoms)
...
def process_image(chem, bo, ds, split, train_set=None, test_set=None):
 ...
 '460', '7787', '279', '1255', '5621']
    elif DB.split('_')[0] == 'omol' or DB.split('_')[0] == 'tg' or DB.split('_')[0] == 'tmqm':
        test_set = test_set
    else:
        print('No test set available for this database')
        test_set = []
    ...
    if DB == 'qm9_2':
        ...
        print('No train set available for this database')
    ...
    if ds.ID.iloc[chem].split('_')[1] in test_set:
        ...
        #print(f"{ds.ID.iloc[chem].split('_')[1]} test len {len(test_set)}, train len {len(train_set)}")
...
def creating_images(start, end, bo, ds, STEP, split=0.1):
    ...
    train_set = []
    test_set = []
    if DB.split('_')[0] == 'omol' or DB.split('_')[0] == 'tg' or DB.split('_')[0] == 'tmqm':

        ID_of_compounds = ds['ID'].apply(lambda x: x.split('_')[1]).drop_duplicates()
        print(sorted(list(set(ID_of_compounds))[:10]))
        test_set = ID_of_compounds.sample(frac=0.1, random_state=42).tolist()
        train_set = ID_of_compounds[~ID_of_compounds.isin(test_set)].tolist()


        #test_set = [str(x).split('_')[1] for x in test_set]
        #train_set = [str(x).split('_')[1] for x in train_set]
        print(f'Train test split have been created randomly. There is {len(train_set)} compounds in train set and {len(test_set)} in test set.')
        print(f'Train set: {train_set[:10]}')
        print(f'\nTest set: {test_set[:10]}')
    if MULTIPROCESS:
        process_image_partial = partial(process_image, bo=bo, ds=ds, split=split, train_set=train_set, test_set=test_set)
        ...
        for chem in range(start, end+1):
            ...
            process_image(chem, bo=bo, ds=ds, split=split, train_set=train_set, test_set=test_set)
...
def welcome_message():
 ...
 ╚═╝  ╚═╝  ╚═════╝  ╚═════╝   ╚═════╝  ╚═╝  ╚═╝ ╚══════╝ ╚═╝     ╚═╝
    ...
    print(f"--- RGBChem version: {1.1} ---")
    print(f"--- https://github.com/all2gos/RGBChem ---")
    print(f"--- http://freacts.pwr.edu.pl ---")
    print(f"--- https://doi.org/10.1021/acs.jctc.5c00291 ---")
